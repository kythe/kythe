...
#
# Imported from https://github.com/bazelbuild/bazel/blob/master/tools/cpp/cc_configure.bzl
# with local modifications to support out requirements.
...
def _get_system_root(repository_ctx):
    """Get System root path on Windows, default is C:\\\Windows."""
...
def _find_cc(repository_ctx):
    ...
    if "CC" in repository_ctx.os.environ:
        ...
        if not cc_name:
            cc_name = "clang"
    if "CLANG" in repository_ctx.os.environ:
        cc_name = repository_ctx.os.environ["CLANG"].strip()
        if not cc_name:
            cc_name = "clang"
