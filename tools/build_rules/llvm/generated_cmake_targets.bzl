def generated_cmake_targets(ctx):
    ...
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "Frontend/OpenMP")
    ctx.set(ctx, "LLVM_TARGET_DEFINITIONS", "OMP.td")
    ctx.tablegen(ctx, "LLVM", "OMP.h.inc", "--gen-directive-decl")
    ctx.tablegen(ctx, "LLVM", "OMP.cpp.inc", "--gen-directive-impl")
    ...
    ctx.add_llvm_component_library(ctx, "LLVMFrontendOpenMP", "OMPConstants.cpp", "OMPContext.cpp", "OMPIRBuilder.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/Frontend", "/root/include/llvm/Frontend/OpenMP", "DEPENDS", "intrinsics_gen", "omp_gen")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.pop_directory(ctx)
