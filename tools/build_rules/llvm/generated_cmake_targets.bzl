def generated_cmake_targets(ctx):
    ...
    ctx.set(ctx, "LLVM_ALL_PROJECTS", "clang;clang-tools-extra;compiler-rt;debuginfo-tests;libc;libclc;libcxx;libcxxabi;libunwind;lld;lldb;llgo;mlir;openmp;parallel-libs;polly;pstl")
    ctx.set(ctx, "LLVM_ENABLE_PROJECTS", "", "CACHE", "STRING", "Semicolon-separated list of projects to build (clang;clang-tools-extra;compiler-rt;debuginfo-tests;libc;libclc;libcxx;libcxxabi;libunwind;lld;lldb;llgo;mlir;openmp;parallel-libs;polly;pstl), or \"all\".")
    ...
    ctx.set(ctx, "LLVM_ENABLE_ZLIB", "", "CACHE", "STRING", "Use zlib for compression/decompression if available. Can be ON, OFF, or FORCE_ON")
    ...
    ctx.add_llvm_component_library(ctx, "LLVMMIRParser", "MILexer.cpp", "MIParser.cpp", "MIRParser.cpp", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "GlobalISel")
    ctx.add_llvm_component_library(ctx, "LLVMGlobalISel", "CSEInfo.cpp", "GISelKnownBits.cpp", "CSEMIRBuilder.cpp", "CallLowering.cpp", "GlobalISel.cpp", "Combiner.cpp", "CombinerHelper.cpp", "GISelChangeObserver.cpp", "IRTranslator.cpp", "InstructionSelect.cpp", "InstructionSelector.cpp", "LegalityPredicates.cpp", "LegalizeMutations.cpp", "Legalizer.cpp", "LegalizerHelper.cpp", "LegalizerInfo.cpp", "Localizer.cpp", "MachineIRBuilder.cpp", "RegBankSelect.cpp", "RegisterBank.cpp", "RegisterBankInfo.cpp", "Utils.cpp", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "BinaryFormat")
    ctx.add_llvm_component_library(ctx, "LLVMBinaryFormat", "AMDGPUMetadataVerifier.cpp", "Dwarf.cpp", "Magic.cpp", "Minidump.cpp", "MsgPackDocument.cpp", "MsgPackDocumentYAML.cpp", "MsgPackReader.cpp", "MsgPackWriter.cpp", "Wasm.cpp", "XCOFF.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/BinaryFormat")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMVectorize", "LoadStoreVectorizer.cpp", "LoopVectorizationLegality.cpp", "LoopVectorize.cpp", "SLPVectorizer.cpp", "Vectorize.cpp", "VPlan.cpp", "VPlanHCFGBuilder.cpp", "VPlanPredicator.cpp", "VPlanSLP.cpp", "VPlanTransforms.cpp", "VPlanVerifier.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/Transforms", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMRemarks", "BitstreamRemarkParser.cpp", "BitstreamRemarkSerializer.cpp", "Remark.cpp", "RemarkFormat.cpp", "RemarkLinker.cpp", "RemarkParser.cpp", "RemarkSerializer.cpp", "RemarkStringTable.cpp", "YAMLRemarkParser.cpp", "YAMLRemarkSerializer.cpp")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMDebugInfoGSYM", "Header.cpp", "FileWriter.cpp", "FunctionInfo.cpp", "GsymCreator.cpp", "GsymReader.cpp", "InlineInfo.cpp", "LineTable.cpp", "LookupResult.cpp", "Range.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/DebugInfo/GSYM", "/root/include/llvm/DebugInfo", "DEPENDS", "LLVMMC")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMJITLink", "JITLink.cpp", "JITLinkGeneric.cpp", "JITLinkMemoryManager.cpp", "EHFrameSupport.cpp", "MachO.cpp", "MachO_arm64.cpp", "MachO_x86_64.cpp", "MachOLinkGraphBuilder.cpp", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMAsmParser", "LLLexer.cpp", "LLParser.cpp", "Parser.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/Analysis", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMPasses", "PassBuilder.cpp", "PassPlugin.cpp", "StandardInstrumentations.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/Passes", "DEPENDS", "intrinsics_gen")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMX86Desc", "X86ATTInstPrinter.cpp", "X86IntelInstPrinter.cpp", "X86InstComments.cpp", "X86InstPrinterCommon.cpp", "X86AsmBackend.cpp", "X86MCTargetDesc.cpp", "X86MCAsmInfo.cpp", "X86MCCodeEmitter.cpp", "X86MachObjectWriter.cpp", "X86ELFObjectWriter.cpp", "X86WinCOFFObjectWriter.cpp", "X86WinCOFFStreamer.cpp", "X86WinCOFFTargetStreamer.cpp")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "TargetInfo")
    ctx.add_llvm_component_library(ctx, "LLVMX86Info", "X86TargetInfo.cpp")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "Utils")
    ctx.add_llvm_component_library(ctx, "LLVMX86Utils", "X86ShuffleDecode.cpp")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_target(ctx, "PowerPCCodeGen", "PPCBoolRetToInt.cpp", "PPCAsmPrinter.cpp", "PPCBranchSelector.cpp", "PPCBranchCoalescing.cpp", "PPCCallingConv.cpp", "PPCCCState.cpp", "PPCCTRLoops.cpp", "PPCHazardRecognizers.cpp", "PPCInstrInfo.cpp", "PPCISelDAGToDAG.cpp", "PPCISelLowering.cpp", "PPCEarlyReturn.cpp", "PPCFastISel.cpp", "PPCFrameLowering.cpp", "PPCLoopInstrFormPrep.cpp", "PPCMCInstLower.cpp", "PPCMachineFunctionInfo.cpp", "PPCMachineScheduler.cpp", "PPCMIPeephole.cpp", "PPCRegisterInfo.cpp", "PPCQPXLoadSplat.cpp", "PPCSubtarget.cpp", "PPCTargetMachine.cpp", "PPCTargetObjectFile.cpp", "PPCTargetTransformInfo.cpp", "PPCTOCRegDeps.cpp", "PPCTLSDynamicCall.cpp", "PPCVSXCopy.cpp", "PPCReduceCRLogicals.cpp", "PPCVSXFMAMutate.cpp", "PPCVSXSwapRemoval.cpp", "PPCExpandISEL.cpp", "PPCPreEmitPeephole.cpp", "PPCLowerMASSVEntries.cpp")
    ctx = ctx.push_directory(ctx, "AsmParser")
    ...
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "MCTargetDesc")
    ctx.add_llvm_component_library(ctx, "LLVMPowerPCDesc", "PPCAsmBackend.cpp", "PPCInstPrinter.cpp", "PPCMCTargetDesc.cpp", "PPCMCAsmInfo.cpp", "PPCMCCodeEmitter.cpp", "PPCMCExpr.cpp", "PPCPredicates.cpp", "PPCMachObjectWriter.cpp", "PPCELFObjectWriter.cpp", "PPCXCOFFObjectWriter.cpp")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "TargetInfo")
