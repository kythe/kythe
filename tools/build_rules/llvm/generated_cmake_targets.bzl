def generated_cmake_targets(ctx):
    ...
    ctx.add_llvm_component_library(ctx, "LLVMInterfaceStub", "ELFObjHandler.cpp", "ELFStub.cpp", "TBEHandler.cpp", "LINK_COMPONENTS", "Object", "Support")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.add_llvm_component_library(ctx, "LLVMLinker", "IRMover.cpp", "LinkModules.cpp", "ADDITIONAL_HEADER_DIRS", "/root/include/llvm/Linker", "DEPENDS", "intrinsics_gen", "LINK_COMPONENTS", "Core", "Support", "TransformUtils")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx.set(ctx, "LLVM_LINK_COMPONENTS", "Support")
    ctx.add_clang_library(ctx, "clangAPINotes", "APINotesYAMLCompiler.cpp", "LINK_LIBS", "clangBasic")
    ctx = ctx.pop_directory(ctx)
