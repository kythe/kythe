def generated_cmake_targets(ctx):
    ...
    ctx.set(ctx, "LLVM_ENABLE_PROJECTS", "", "CACHE", "STRING", "Semicolon-separated list of projects to build (clang;clang-tools-extra;compiler-rt;debuginfo-tests;libc;libclc;libcxx;libcxxabi;libunwind;lld;lldb;mlir;openmp;parallel-libs;polly;pstl), or \"all\".")
    ...
    ctx.add_llvm_component_library(ctx, "LLVMRemarks", "BitstreamRemarkParser.cpp", "BitstreamRemarkSerializer.cpp", "Remark.cpp", "RemarkFormat.cpp", "RemarkLinker.cpp", "RemarkParser.cpp", "RemarkSerializer.cpp", "RemarkStreamer.cpp", "RemarkStringTable.cpp", "YAMLRemarkParser.cpp", "YAMLRemarkSerializer.cpp")
    ctx = ctx.pop_directory(ctx)
    ...
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "MCTargetDesc")
    ctx.add_llvm_component_library(ctx, "LLVMPowerPCDesc", "PPCAsmBackend.cpp", "PPCInstPrinter.cpp", "PPCMCTargetDesc.cpp", "PPCMCAsmInfo.cpp", "PPCMCCodeEmitter.cpp", "PPCMCExpr.cpp", "PPCPredicates.cpp", "PPCMachObjectWriter.cpp", "PPCELFObjectWriter.cpp", "PPCXCOFFObjectWriter.cpp", "PPCELFStreamer.cpp")
    ctx = ctx.pop_directory(ctx)
    ctx = ctx.push_directory(ctx, "TargetInfo")
    ...
    ctx.set(ctx, "openmp_wrapper_files", "openmp_wrappers/math.h", "openmp_wrappers/cmath", "openmp_wrappers/__clang_openmp_math.h", "openmp_wrappers/__clang_openmp_math_declares.h", "openmp_wrappers/new")
