...
type buildConfig struct {
	...
	preProcess func(configFile string) error
}
...
func verifyFlags() (config buildConfig) {
	...
	case "MAVEN":
		...
		config.preProcess = preProcessMavenBuild
	case "GRADLE":
		hasError = hasError || verifyGradleFlags()
		config.buildFile = *gradleBuildFile
		config.preProcess = wrapper.PreProcessGradleBuild
	default:
		hasError = true
	...
	if config.preProcess == nil {
		config.preProcess = func(configFile string) error {
...
func main() {
	...
	if err := config.preProcess(config.buildFile); err != nil {
