...
COPY --from=javac-extractor-artifacts /opt/kythe/extractors/javac8_extractor.jar /opt/kythe/extractors/javac8_extractor.jar
ENV KYTHE_CORPUS=testcorpus
ENV REAL_JAVAC=$JAVA_HOME/bin/javac
ENV JAVAC_EXTRACTOR_JAR=/opt/kythe/extractors/javac8_extractor.jar
ENV KYTHE_OUTPUT_FILE=$KYTHE_OUTPUT_DIRECTORY/extractor-output.kzip
ENV KYTHE_JAVA_RUNTIME_OPTIONS=-Xbootclasspath/p:$JAVAC_EXTRACTOR_JAR
