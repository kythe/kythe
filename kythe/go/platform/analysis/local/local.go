...
// Analyzer provides an implementation of the analysis.CompilationAnalyzer
...
// NewAnalyzer creates an analyzer that will execute the given command when
...
func NewAnalyzer(cmd []string, fetcher analysis.Fetcher) *Analyzer {
	if len(cmd) == 0 {
		panic("parameter 'cmd' must not be empty")
	}
	return &Analyzer{cmd: cmd, fetcher: fetcher}
}
...
func (a *Analyzer) Analyze(ctx context.Context, req *apb.AnalysisRequest, f analysis.OutputFunc) error {
	...
	if e, ok := err.(*exec.ExitError); ok {
		...
		return fmt.Errorf("indexing kzip %s: %v", tmpkzip, err)
	}

	// Deserialize output and pass it to `f`.
	rd := delimited.NewReader(bytes.NewReader(out))
	for {
		n, err := rd.Next()
		if err == io.EOF {
			break
