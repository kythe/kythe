...
impl<'a> ProxyWriter<'a> {
    /// Create a new instance of ProxyWriter
    ///
    /// Takes a writer that implements the `Write` trait as the only argument.
    pub fn new() -> ProxyWriter<'a> {
        Self {
            buffer: Vec::new(),
        }
    }
...
impl<'a> KytheWriter for ProxyWriter<'a> {
    ...
    fn flush(&mut self) -> Result<(), KytheError> {
        ...
        io::stdout::flush()
            .map_err(|err| KytheError::WriterError(format!("Failed to flush stdout: {:?}", err)))?;
        ...
        if response["rsp"].as_str() == "error" {
            Err(KytheError::WriterError(
                format!("Got error back from proxy after sending entries: {}", response["args"].as_str())
            ))
        }
