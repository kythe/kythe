...
// Binary extract extracts a pre-configured OpenJDK21 source tree to produce Kythe compilation units.
package main
...
const (
	...
	kytheExcludeVar = "KYTHE_OPENJDK21_EXCLUDE_MODULES"

	javaMakeVar           = "JAVA_CMD"
	runfilesPrefix        = "${RUNFILES}"
	runfilesWrapperPath   = "kythe/extractors/openjdk21/java_wrapper/java_wrapper"
	runfilesVNamesPath    = "kythe/extractors/openjdk21/vnames.json"
...
func defaultOutputDir() string {
	...
	if val == "" {
		...
		val = filepath.Join(usr.HomeDir, "kythe-openjdk21-output")
	}
	return val
}
...
func makeEnv() []string {
	...
	env = setEnvDefault(env, kytheCorpusVar, "openjdk21")
...
func init() {
	setupRunfiles()
	flag.StringVar(&sourceDir, "jdk", "", "path to the OpenJDK21 source tree (required)")
	flag.StringVar(&buildDir, "build", "", "path to the OpenJDK21 build tree (defaults to -jdk)")
	...
	flag.Usage = flagutil.SimpleUsage("Extract a configured openjdk21 source directory", "[--java_wrapper=] [path]")
}
