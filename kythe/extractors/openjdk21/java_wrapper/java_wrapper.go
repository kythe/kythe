...
const (
	...
	excludeModulesVar = "KYTHE_OPENJDK21_EXCLUDE_MODULES" // Names of for which to skip extraction.
)

var (
	moduleInfoPattern = regexp.MustCompile("^.*/([^/]+)/module-info.java$")
)

func moduleName() string {
	path := os.Args[len(os.Args)-1]
	// Hackish way to determine the likely module being compiled.
	file, err := os.Open(strings.TrimPrefix(path, "@"))
	if err != nil {
		log.Exitf("failed to open %s: %v", path, err)
	}
	defer file.Close()
	scanner := bufio.NewScanner(file)
	module, err := moduleNameFromPath(scanner)
	if err != nil {
		log.Errorf("No modules found from file contents of path - %s. Using dirname %s instead.",
			path, module)
		// No match, use dirname to form the module.
		module = filepath.Base(filepath.Dir(path))
	}
	return module
}

func moduleNameFromPath(scanner *bufio.Scanner) (string, error) {
	var module string
	for scanner.Scan() {
		match := moduleInfoPattern.FindStringSubmatch(strings.TrimSpace(scanner.Text()))
		if match != nil {
			if module == "" {
				module = match[1]
			} else if match[1] != module {
				return "", fmt.Errorf("multiple modules found: %s & %s", module, match[1])
			}
		}
	}
	if module == "" {
...
func main() {
	...
	if args := extractorArgs(os.Args[1:], jar); len(args) > 0 {
		var moduleNameString = moduleName()
		if excludeModules.Contains(moduleNameString) {
			log.Infof("*** Skipping: %s", moduleNameString)
		} else {
