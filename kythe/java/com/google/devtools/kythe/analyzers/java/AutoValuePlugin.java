...
import com.google.devtools.kythe.analyzers.java.AutoValueClass.GeneratedSymbol;
import com.google.devtools.kythe.analyzers.java.AutoValueClass.Property;
...
public class AutoValuePlugin extends Plugin.Scanner<Void, Void> {
  ...
  private void emitAutoValue(AutoValueClass autoValue) {
    ...
    for (Property prop : autoValue.properties()) {
      ...
      if (getterNode.isPresent()) {
        ...
        prop.getter().stream()
            .forEach(
                sym -> {
                  Optional<KytheNode> kytheNode = kytheGraph.getNode(sym);
                  entrySets.emitEdge(
        ...
        Streams.stream(prop.setter())
            ...
            .forEach(
                sym -> {
                  Optional<KytheNode> kytheNode = kytheGraph.getNode(sym);
                  entrySets.emitEdge(
                      kytheNode.get().getVName(), EdgeKind.PROPERTY_WRITES, propNode.getVName());
                });
      }
    }
  }

  private JCClassDecl findAutoValueDecl(ClassType classType, String annotationName) {
  ...
  private Optional<AutoValueClass> resolveGeneratedAutoValue(JCClassDecl classDef) {
    ...
    JCClassDecl autoValueClass =
        findAutoValueDecl((ClassType) classDef.type, AUTO_VALUE_ANNOTATION);
    if (autoValueClass == null) {
      return Optional.empty();
    }

    // Construct base AutoValueClass.
    AutoValueClass.Builder autoValue =
        AutoValueClass.builder()
            .setSymbol(
    ...
    for (JCTree def : classDef.getMembers()) {
      if (def instanceof JCMethodDecl) {
        ...
        JCClassDecl builderClass = findAutoValueDecl((ClassType) subClass.type, BUILDER_ANNOTATION);
    ...
    if (getters.keySet().stream().allMatch(AutoValuePlugin::hasGetPropertyPrefix)) {
