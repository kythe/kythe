  ...
  public ListenableFuture<byte[]> startLookup(String path, String digest) {
    if (path == null && digest == null) {
      return Futures.immediateFailedFuture(
          new IllegalArgumentException("path and digest cannot be null"));
    }
    SettableFuture<byte[]> future = SettableFuture.create();

    FilesRequest.Builder req = FilesRequest.newBuilder();
    Analysis.FileInfo.Builder fileInfoBuilder = req.addFilesBuilder();
    if (path != null) {
      fileInfoBuilder.setPath(path);
    }
    if (digest != null) {
      fileInfoBuilder.setDigest(digest);
    }
