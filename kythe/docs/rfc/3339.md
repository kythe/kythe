...
Analysis types are described by the following grammar:

```grammatical-framework
analysis-type         = analysis-type-segment 0*{"/" analysis-type-segment}
                
analysis-type-segment = unreserved -- RFC 3987
```
...
-   `analyze(AnalyzeRequest) → AnalyzeReply`
    ...
    objects. Other analyziers might emit a "findings" structure.
...
« {"jsonrpc":"2.0", "id":2, "result":{"id":101, "input":"path/to/file1.go"}}
» {"jsonrpc":"2.0", "id":3, "method":"analysis", "params":{"types":["kythe/indexer/go"]}}
« {"jsonrpc":"2.0", "id":3, "result:"{"id":384, "input":"path/to/file2.go"}}
