// Code generated by protoc-gen-go. DO NOT EDIT.
// source: kythe/proto/buildinfo.proto

/*
Package buildinfo_go_proto is a generated protocol buffer package.

It is generated from these files:
	kythe/proto/buildinfo.proto

It has these top-level messages:
	BuildDetails
*/
package buildinfo_go_proto

import proto "github.com/golang/protobuf/proto"
import fmt "fmt"
import math "math"

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion2 // please upgrade the proto package

type BuildDetails struct {
	BuildTarget string `protobuf:"bytes,1,opt,name=build_target,json=buildTarget" json:"build_target,omitempty"`
	RuleType    string `protobuf:"bytes,2,opt,name=rule_type,json=ruleType" json:"rule_type,omitempty"`
}

func (m *BuildDetails) Reset()                    { *m = BuildDetails{} }
func (m *BuildDetails) String() string            { return proto.CompactTextString(m) }
func (*BuildDetails) ProtoMessage()               {}
func (*BuildDetails) Descriptor() ([]byte, []int) { return fileDescriptor0, []int{0} }

func (m *BuildDetails) GetBuildTarget() string {
	if m != nil {
		return m.BuildTarget
	}
	return ""
}

func (m *BuildDetails) GetRuleType() string {
	if m != nil {
		return m.RuleType
	}
	return ""
}

func init() {
	proto.RegisterType((*BuildDetails)(nil), "kythe.proto.BuildDetails")
}

func init() { proto.RegisterFile("kythe/proto/buildinfo.proto", fileDescriptor0) }

var fileDescriptor0 = []byte{
	// 150 bytes of a gzipped FileDescriptorProto
	,
}
